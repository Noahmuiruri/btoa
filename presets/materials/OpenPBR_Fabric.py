...
# OpenPBR Fabric preset - Velvet
node.geometry_thin_walled = False
node.caustics = False

# Base - fabric base
node.inputs["base_weight"].default_value = 1.0
node.inputs["base_color"].default_value = (0.3, 0.1, 0.5)
node.inputs["base_metalness"].default_value = 0.0
node.inputs["base_diffuse_roughness"].default_value = 1.0

# Specular - minimal specular
node.inputs["specular_weight"].default_value = 0.3
node.inputs["specular_color"].default_value = (1.0, 1.0, 1.0)
node.inputs["specular_roughness"].default_value = 0.8
...
# Fuzz - key feature for fabric
node.inputs["fuzz_weight"].default_value = 1.0
node.inputs["fuzz_color"].default_value = (0.4, 0.15, 0.6)
node.inputs["fuzz_roughness"].default_value = 0.3
